...
// Server-side step-up secret for sensitive areas (CSAM / DM review / silent
// investigation). It is NEVER hardcoded or persisted — the admin enters it at
// runtime and it lives only in memory for the session, sent as the X-Step-Up header.
let stepUpSecret = ''
export const setStepUp = (s: string) => { stepUpSecret = (s || '').trim() }
export const hasStepUp = () => !!stepUpSecret
const stepUpHeaders = (): Record<string, string> => stepUpSecret ? { 'X-Step-Up': stepUpSecret } : {}
...
export const api = {
  ...
  // Block 5 — discovery, offers, storefront, subscribe/tip
  choices: () => req('/choices'),
  choicesOptIn: () => req('/choices/opt-in', { method: 'POST' }),
  tagsSimilar: (q: string) => req(`/tags/similar?q=${encodeURIComponent(q)}`),
  tagsTrending: () => req('/tags/trending'),
  nsfwTags: () => req('/nsfw-tags'),
  createSponsored: (post_id: string, topics?: string[]) => req('/sponsored', { method: 'POST', body: j({ post_id, topics }) }),
  creatorOffers: (handle: string) => req(`/creators/${handle}/offers`),
  setCreatorOffers: (b: any) => req('/creator/offers', { method: 'PUT', body: j(b) }),
  creatorPublicShop: (handle: string) => req(`/creators/${handle}/shop`),
  checkoutSession: (b: any) => req('/checkout/session', { method: 'POST', body: j(b) }),
  shopOrder: (productId: string) => req(`/shop/order/${productId}`, { method: 'POST' }),
  ...
  followInterest: (name: string) => req(`/interests/${encodeURIComponent(name)}`, { method: 'POST' }),
  unfollowInterest: (name: string) => req(`/interests/${encodeURIComponent(name)}`, { method: 'DELETE' }),
  interestPeople: (name: string) => req(`/interests/${encodeURIComponent(name)}/people`),
  dmThreads: () => req('/dms'),
  ...
  adminCsam: () => req('/admin/csam', { headers: stepUpHeaders() }),
  ...
  adminUserDms: (handle: string, legal_basis: string) => req(`/admin/dms/${handle}?legal_basis=${encodeURIComponent(legal_basis)}`, { headers: stepUpHeaders() }),
  adminInvestigate: (handle: string, legal_basis: string) => req(`/admin/investigate/${handle}?legal_basis=${encodeURIComponent(legal_basis)}`, { headers: stepUpHeaders() }),
  ...
  adminCsamEscalate: (id: string) => req(`/admin/csam/${id}/escalate`, { method: 'POST', headers: stepUpHeaders() }),
  adminCsamResolve: (id: string) => req(`/admin/csam/${id}/resolve`, { method: 'POST', headers: stepUpHeaders() }),
}
