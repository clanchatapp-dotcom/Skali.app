...
export default function Admin() {
  ...
  const runInvestigation = async () => {
    ...
    const basis = promptLegalBasis()
    if (!basis) return
    setInvLoading(true); setInv(null)
    try { setInv(await api.adminInvestigate(h, basis)) } catch (e: any) { alert(e.message) } finally { setInvLoading(false) }
  }
  ...
  const load = async () => {
    ...
    try {
      ...
      else if (tab === 'csam') { if (!ensureStepUp()) { setData([]); setLoading(false); return } setData(await api.adminCsam()) }
  ...
  const csamEscalate = async (id: string) => { if (!ensureStepUp()) return; try { const r = await api.adminCsamEscalate(id); alert(`Escalated. Reference: ${r.ceop_ref}`); await load() } catch (e: any) { alert(e.message) } }
  const csamResolve = async (id: string) => { if (!ensureStepUp()) return; await api.adminCsamResolve(id); await load(); await loadStats() }
  ...
  const ensureStepUp = (): boolean => {
    if (hasStepUp()) return true
    const s = window.prompt('Sensitive area. Enter the server-side step-up secret to continue:')
    if (!s || !s.trim()) return false
    setStepUp(s.trim())
    return true
  }
  const promptLegalBasis = (): string | null => {
    const b = window.prompt('State the legal basis for this access (e.g. warrant / court order ref, safeguarding case). Min 8 characters. This is recorded in the audit log:')
    if (!b || b.trim().length < 8) { if (b !== null) alert('A legal basis of at least 8 characters is required.'); return null }
    return b.trim()
  }
  const viewDms = async (handle: string) => {
    ...
    try { setDm(await api.adminUserDms(handle, basis)) } catch (e: any) { alert(e.message); setDm(null) } finally { setDmLoading(false) }
  }
