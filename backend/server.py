...
async def invite_inner(handle: str, u: dict = Depends(get_current_user)):
    ...
    if await adult_minor_barrier(u['id'], member['id']):
        raise HTTPException(403, 'This account is not available to you')  # child-safety wall
    # One invite only: if a relationship already exists (invited or accepted),
    # do NOT re-send or spam the recipient's activity feed with duplicate invites.
    existing = await db.inner.find_one({'owner_id': u['id'], 'member_id': member['id']})
    if existing:
        return {'status': existing.get('status', 'pending')}
    await db.inner.insert_one({'owner_id': u['id'], 'member_id': member['id'],
                               'status': 'pending',
                               'created_at': datetime.now(timezone.utc).isoformat()})
...
async def accept_inner(handle: str, u: dict = Depends(get_current_user)):
    ...
    if not owner:
        raise HTTPException(404, 'Not found')
    inv = await db.inner.find_one({'owner_id': owner['id'], 'member_id': u['id']})
    if not inv:
        raise HTTPException(400, 'No invite to accept')
    # Idempotent: only record the join activity on the first accept, never on repeats.
